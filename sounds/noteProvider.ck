public class NP {
    ...
    fun static int size() { return octaves * notes.size(); }

    fun static int getNote(float x) {
        // x in [0, 1)
    ...
    fun static float freqToMidi(float hz) { return 69.0 + 12.0 * Math.log2(hz / 440.0); }

    ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"] @=> static string chroma[];

    // returns a friendly note name for a given frequency
    fun static string freqToNoteName(float hz) {
